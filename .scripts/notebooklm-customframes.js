// NotebookLM to Obsidian Auto-Saver (Custom Frames Edition)
// Version: 2.2.0 (converted from Tampermonkey UserScript)
//
// ⚠️ 주의: 이 파일은 Custom Frames의 customJs 필드에 직접 사용하기 위한 버전입니다.
// Obsidian 설정 → Custom Frames → NotebookLM → Additional JavaScript에 복사하여 사용하세요.
...
(function() {
    ...
    // ========================================
    const CONFIG = {
        obsidianApiUrl: 'http://127.0.0.1:27123',  // HTTP (CORS 문제 회피)
...
`;
    ...
    /**
     * Obsidian에 노트 저장
     * ⚠️ Tampermonkey의 GM_xmlhttpRequest 대신 fetch API 사용
     */
    function saveToObsidian(title, markdown) {
        return new Promise((resolve, reject) => {
            ...
            // fetch API 사용 (GM_xmlhttpRequest 대체)
            fetch(`${CONFIG.obsidianApiUrl}/vault/${encodeURIComponent(filepath)}`, {
                method: 'PUT',
                headers: {
    ...
    // 페이지 로드 시 버튼 감지
    function init() {
        console.log('[NotebookLM→Obsidian] 스크립트 시작 (v2.2.0 - Custom Frames Edition)');
        ...
        console.log('[NotebookLM→Obsidian] 초기화 완료 (Custom Frames)');
    }
