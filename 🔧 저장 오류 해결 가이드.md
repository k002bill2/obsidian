...
> 💡 `$OBSIDIAN_API_KEY`는 `~/.zshrc`에 정의된 환경 변수입니다.
> 직접 확인: `echo $OBSIDIAN_API_KEY`
...
| 위치 | 설명 |
|------|------|
| `~/.zshrc` | 환경 변수 `$OBSIDIAN_API_KEY` |
| `Secret/Token.md` | Obsidian 내 참조용 |
| 플러그인 설정 | Local REST API → API Key |
| 스크립트 | `.scripts/notebooklm-customframes.js` → `OBSIDIAN_API_KEY` 상수 |
